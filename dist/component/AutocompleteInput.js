        ...
        if (divShowing) {
            if (UserAgent.isSafari()) {
                ...
                if (inputRect.top < window.visualViewport.height / 2) {
                    divRef.current.style.top = StringObject.join(["calc(", inputRect.bottom, "px + 0.3em)"]).toString();
                }
                else {
                    divRef.current.style.bottom = StringObject.join(["calc(100vh - ", inputRect.top, "px + 0.3em)"]).toString();
                }
                divRef.current.style.width = inputRect.width + "px";
                divRef.current.style.maxHeight = "9em";
            }
